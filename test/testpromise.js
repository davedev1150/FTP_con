...
async function connectToFTP(coninfo) {
  const client = new ftp();
  let resultsArray = [];
  let status_con = false;

  client.on("ready", () => {
    console.log("Connected to FTP server", coninfo.name);
    status_con = true;
    for (const dirPath of coninfo.dir) {
      ...
      client.cwd(dirPath, (err, newCurrentDir) => {
        if (err) {
          ...
          client.list((err, newList) => {
            if (err) {
              console.error(
                `Error listing remote directory at ${coninfo.name}:`,
                err
              );
            } else {
              console.log(`Remote directory listing at ${dirPath} ${coninfo.name}:`);
              ...
              if (fileToDownload && fileToDownload.type === "-") {
                    ...
                    stream.on("end", () => {
                      ...
                      if (results.length > 1) {
                        ...
                        if (filteredHeaders.length > 1) {
                          console.log(
                            `${coninfo.name} is NAN!!!! from ${dirPath}`,
                            filteredHeaders,
                            fileToDownload.name
                          );
                          resultsArray.push({
                          ...
                          console.log(`${server.name} not NAN`);
                          resultsArray.push({
                            name: coninfo.name,
                            code_name: coninfo.code_name,
                            be_under: coninfo.be_under,
                ...
                console.log(`fileToDownload error`);
              }
            }
          });
        }
...
const coninfo = {
  ...
  dir: ["/Omnia1", "/Omnia2"], // Add the directories you want to process
};

connectToFTP(coninfo);
